# 采用grid search分析不同learning rate和batch size
...
class DIRLs(nn.Module):
    ...
    def update(self, minibatches, val_dataset, batchnum, epochs=30, lr=0.001):
        ...
        self.to(device)
        self.train()
        print('本轮优化所用学习率：%f, batchsize: %d' %(args.lr, args.batchSize))
        # 复制epoch个迭代器
...
if __name__ == "__main__":
    ...
    for i, lr in enumerate(learning_rates):
        for j, bs in enumerate(batch_sizes):
            ...
            testloader = DataLoader(dataset4, batch_size=args.batchSize, shuffle=False, drop_last=True)
            # 初始化模型
            model = ConvNet()
            model1 = DIRLs(args)
            # 融合多个dataset
            ...
            MyDirls.teanettrain(dataloaders=minibatch[0], lr=args.lr)
            print('教师模型训练完毕...\n')
